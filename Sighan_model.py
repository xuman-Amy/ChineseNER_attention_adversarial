...
class TransferModel(object):
    ...
    def adversarial_loss(self, feature):
        ...
        W_adv = tf.get_variable(name='W_adv',
                                shape=[2 * self.lstm_dim, self.task_num],
                                dtype=tf.float32,
                                initializer=tf.contrib.layers.xavier_initializer())
        b_adv = tf.get_variable(name='b_adv',
                                shape=[self.task_num],
                                dtype=tf.float32,
                                initializer=tf.contrib.layers.xavier_initializer())
        logits = tf.nn.xw_plus_b(feature, W_adv, b_adv)
        adv_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=logits,
    ...
    def self_attention(self, keys, scope='multihead_attention', reuse=None):
        with tf.variable_scope(scope, reuse=reuse):
            Q = tf.nn.relu(
                tf.layers.dense(keys, self.num_units,
    ...
    def multi_task(self):
        ...
        with tf.variable_scope('common_bilstm'):
            ...
            if self.is_train:
                shared_cell_fw = tf.nn.rnn_cell.DropoutWrapper(shared_cell_fw,
                                                               input_keep_prob=self.in_keep_prob,
                                                               output_keep_prob=self.out_keep_prob)
                shared_cell_bw = tf.nn.rnn_cell.DropoutWrapper(shared_cell_bw,
                                                               input_keep_prob=self.in_keep_prob,
                                                               output_keep_prob=self.out_keep_prob)
        ...
        with tf.variable_scope('cws_private_bilstm'):
            ...
            if self.is_train:
                cws_private_cell_fw = tf.nn.rnn_cell.DropoutWrapper(cws_private_cell_fw,
                                                                    input_keep_prob=self.in_keep_prob,
                                                                    output_keep_prob=self.out_keep_prob)
                cws_private_cell_bw = tf.nn.rnn_cell.DropoutWrapper(cws_private_cell_bw,
                                                                    input_keep_prob=self.in_keep_prob,
                                                                    output_keep_prob=self.out_keep_prob)
        ...
        with tf.variable_scope('ner_private_bilstm'):
            ...
            if self.is_train:
                ner_private_cell_fw = tf.nn.rnn_cell.DropoutWrapper(ner_private_cell_fw,
                                                                    input_keep_prob=self.in_keep_prob,
                                                                    output_keep_prob=self.out_keep_prob)
                ner_private_cell_bw = tf.nn.rnn_cell.DropoutWrapper(ner_private_cell_bw,
                                                                    input_keep_prob=self.in_keep_prob,
                                                                    output_keep_prob=self.out_keep_prob)
        ...
        W_ner = tf.get_variable(name='W_ner',
                                shape=[4 * self.lstm_dim, self.lstm_dim],
                                dtype=tf.float32,
                                initializer=tf.contrib.layers.xavier_initializer())
        b_ner = tf.get_variable(name='b_ner',
                                shape=[self.lstm_dim],
                                dtype=tf.float32,
                                initializer=tf.contrib.layers.xavier_initializer())
        ...
        logits_W = tf.get_variable(name='logits_weight',
                                   shape=[self.lstm_dim, self.ner_tags_num],
                                   dtype=tf.float32)
        logits_b = tf.get_variable(name='logits_bias',
                                   shape=[self.ner_tags_num],
                                   dtype=tf.float32)
        ...
        output = tf.reshape(output, [-1, 4 * self.lstm_dim])
        W_cws = tf.get_variable(name='W_cws',
                                shape=[4 * self.lstm_dim, self.lstm_dim],
                                dtype=tf.float32,
                                initializer=tf.contrib.layers.xavier_initializer())
        b_cws = tf.get_variable(name='b_cws',
                                shape=[self.lstm_dim],
                                dtype=tf.float32,
                                initializer=tf.contrib.layers.xavier_initializer())
        ...
        if self.is_train:
            hidden_output = tf.nn.dropout(hidden_output, self.keep_prob1)
        logits_W_cws = tf.get_variable(name='cws_weight',
                                       shape=[self.lstm_dim, self.cws_tags_num],
                                       dtype=tf.float32)
        logits_b_cws = tf.get_variable(name='cws_bias',
                                       shape=[self.cws_tags_num],
                                       dtype=tf.float32)
        ...
        self.loss = tf.cast(self.is_ner, tf.float32) * self.ner_loss \
                    + tf.cast((1 - self.is_ner), tf.float32) * self.cws_loss \
                    + self.adv_weight * self.adv_loss
